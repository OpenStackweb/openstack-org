 ...
 **/
class SangriaSurveyBuilderRouterApiExtension extends DataExtension
{
    const SubRouteSurveyTemplates = 'survey-templates';

    static $allowed_subroutes = [
        self::SubRouteSurveyTemplates
    ];
    ...
    public function handleSubRoute($subroute, SS_HTTPRequest $request){
        if(!empty($subroute) && in_array($subroute, self::$allowed_subroutes)){
            ...
            switch($subroute){
                ...
                case self::SubRouteSurveyTemplates:
                    $api = SangriaSurveyTemplateApi::create();
                    break;
            }
            if(!is_null($api))

                ...
                <% if PrivateClouds %>
                    ...
                    <tr>
                        ...
                        <td style="min-width: 200px" width="30%">
                            <a class="product-button roundedButton addDeploymentBtn" href="$Top.Link(private_cloud)?id=$ID&is_draft=$isDraft">Edit</a>
                            <% if isDraft() %>
                                <a target="_blank" class="product-button roundedButton addDeploymentBtn" href="$Top.Link(private_cloud)/$ID/draft_preview">Preview Draft</a>
                                <a target="_blank" class="product-button roundedButton addDeploymentBtn" href="$Top.Link(private_cloud)/$ID/draft_pdf">PDF</a>
                            <% else %>
                                <a target="_blank" class="product-button roundedButton addDeploymentBtn" href="$Top.Link(private_cloud)/$ID/preview">Preview Live</a>
                                <a target="_blank" class="product-button roundedButton addDeploymentBtn" href="$Top.Link(private_cloud)/$ID/pdf">PDF</a>
                            <% end_if %>
                            <a class="roundedButton delete-private-cloud product-button addDeploymentBtn" href="#" data-id="{$ID}" data-is_draft="{$isDraft}">Delete</a>
                        </td>
                    </tr>
                    <% end_loop %>
                <% end_if %>

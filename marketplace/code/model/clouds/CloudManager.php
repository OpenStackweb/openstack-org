...
abstract class CloudManager extends OpenStackImplementationManager
{
    ...
    {
        $cloud = parent::clearCollections($cloud);
    ...
    {
        $cloud = parent::updateCollections($cloud, $data);
    ...
    protected function addDataCenterRegion(array $region_data, ICloudService $cloud)
    {
        ...
        $region = $this->factory->buildDataCenterRegion($region_data['name'], $region_data['color'],
            $region_data['endpoint']);
    ...
    protected function addCapability(array $capability_data, IOpenStackImplementation $company_service)
    {
        ...
        if (!$component) {
            throw new NotFoundEntityException('OpenStackComponent',
                sprintf('id %', intval($capability_data['component_id'])));
        }

        $release = $this->release_repository->getById(intval($capability_data['release_id']));
        if (!$release) {
            throw new NotFoundEntityException('OpenStackRelease',
                sprintf('id %', intval($capability_data['release_id'])));
        }
        ...
        if ($component->getSupportsVersioning()) {
            ...
            if (!$version) {
                throw new NotFoundEntityException('OpenStackApiVersion',
                    sprintf('id %', intval($capability_data['version_id'])));
            }
        ...
        $service = $this->factory->buildCapability(intval($capability_data['coverage']), $supported_version,
            $company_service);

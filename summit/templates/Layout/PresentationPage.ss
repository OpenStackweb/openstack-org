		...
		<div class="row">
			...
			<div class="col-lg-9 col-md-9">
				<div class="presentation-main-panel">
					...
					<table class="table">
						<tbody>
					<% if $CurrentMember.CurrentSpeakerProfile.MyPresentations %>
						<% loop $CurrentMember.CurrentSpeakerProfile.MyPresentations %>
							<tr>
								<td class="item-name"><i class="fa fa-file-text-o"></i><a href="$EditLink">$Title</a></td>
               				    <% if $Status %>
					...
					<table class="table">
						<tbody>
					<% if $CurrentMember.CurrentSpeakerProfile.OtherPresentations %>
						<% loop $CurrentMember.CurrentSpeakerProfile.OtherPresentations %>
							<tr>
								<td class="item-name"><i class="fa fa-file-text-o"></i><a href="$EditLink">$Title</a></td>
                				<td class="status"><i class="fa fa-tag"></i> $Status</td>
								<td class="action"><% if $CanDelete %><a href="$DeleteLink">Delete</a><% end_if %></td>
							</tr>
						<% end_loop %>
					<% else %>
							<tr>

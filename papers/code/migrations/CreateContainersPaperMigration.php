...
final class CreateContainersPaperMigration extends AbstractDBMigrationTask
{
    ...
    function doUp()
    {
        ...
        $section_1_contents = [
          '<p>Imagine that you are tasked to build an entire private cloud infrastructure from the ground up. You have a limited budget, a small but dedicated team, and are asked to pull off a miracle.</p>',
        ...
        $section2->Title = "I. A High Level View of Containers in OpenStack";
        $section2->Order = 2;
...
<p>The final option for building OpenStack application containers is LOCI. LOCI also builds Docker containers, and delivers one container for each project. LOCI is focused on producing compact and secure containers quickly, for all common distributions, with the expectation that they will be used as a foundation to build upon by the deployment system.
            ...
            '<p>This opens up an opportunity to not just run OpenStack virtualization on a bare-metal cloud, but to also run bare-metal Kubernetes-only installations that can take full advantage of the identity, storage, networking, and other cloud APIs available through OpenStack services.</p>',
            ...
            '<p class="text-center"><img src="https://www.openstack.org/themes/openstack/images/containers2/kata-diagram5-r2.svg" alt="Intro" class="hover-shadow clickable-image"></p>',
...
<p>Two standalone Cinder drivers. A Flex Volume driver uses an exec-based model to interface with drivers, and a Container Storage Interface (CSI) driver which uses a standard interface for container orchestration systems to expose arbitrary storage systems to their container workloads. With support for over 70 storage drivers, these drivers make it possible to interface a wealth of battle tested proprietary and open source storage devices through a single Cinder API.</p></li><li><p>A webhook-based authentication and authorization interface to Keystone. Each mode, authentication and authorization, can be configured independently of one another. Though a work in progress, the interface supports a soft-multi-tenancy that backs Kubernetes RBAC with OpenStack Keystone.</p>
        ...
        $section_4_contents = [
            '<p>Many members of the OpenStack community are contributing new code to various OpenStack projects relevant to containers, evaluating the implications and benefits of containers, and using containers in production to solve challenges and unlock new capabilities. This section highlights some of the most interesting case studies.</p>',
        ];
        ...
        // AT&T
        $case1 =  new CaseOfStudy();
        $case1->Title = "AT&T";
        $case1->Order = 1;
        $case1->SectionID = $section4->ID;
        $case1->write();

        $case1_contents = [
            '<p>AT&T, one of the largest telecommunications companies in the world, leverages container technology to deploy and manage OpenStack itself, relying on infrastructure containers to generate simplicity and efficiency, with the aim of building their 5G infrastructure on containerized OpenStack.</p>',
            ...
            '<p class="text-center"><img class="medium-img" src="https://www.openstack.org/themes/openstack/images/containers2/att-diagram.jpg" alt="AT&#38;T"></p>',
            ...
            '<p>Leveraging container technology to deploy and manage OpenStack shouldn’t have much obvious impact on tenants — with the exception that they will have a more highly resilient platform, and will be able to get cloud features more frequently and with minimal interruption.  AT&T’s operations teams new experience will shift more of their efforts to defining the declarative configuration for a site, and to let the Kubernetes-oriented automation carry out the deployments themselves.</p>',
            '<p>AT&T aims to use this architecture to power the virtual network functions that form the backbone of its consumer and business-focused products and services. The initial use case for AT&T’s containerized Network Cloud will be the initial deployment of VNFs for the emerging 5G networking. OpenStack has been, is, and will be an excellent fit for AT&T’s VNF-focused cloud use cases. Containerization is simply an evolution that allows AT&T to deploy, manage, and scale their OpenStack infrastructure in a more reliable, rapid, zero-touch manner.</p>',
            '<p>Operationally, AT&T is still testing this approach but has committed to getting 5G service into production before the end of the year. OpenStack and container technology will form the backbone of this service, which is strategically important for AT&T’s millions of users. Deploying their 5G service will demonstrate the relevance of OpenStack and containers in a massively distributed production environment.</p>',
        ];

        $idx = 1;
        foreach($case1_contents as $content){
            $c = new CaseOfStudyParagraph();
            $c->Content = self::cleanContent($content);
            $c->Order = $idx;
            $c->CaseOfStudyID = $case1->ID;
            $c->write();
            $idx++;
        }

        // CERN
        $case2 =  new CaseOfStudy();
        $case2->Title = "Cern";
        $case2->Order = 2;
        $case2->SectionID = $section4->ID;
        $case2->write();

        $case2_contents = [
          '<p>CERN, the European Organization for Nuclear Research, enables physicists and engineers to probe the fundamental structure of the universe, using the world’s largest and most complex scientific instruments to study the basic constituents of matter – the fundamental particles. The CERN cloud provides physicists with compute resources for scientific computing, analyzing data coming from the Large Hadron Collider and other experiments.</p>',
          '<p>CERN has been running OpenStack in production since 2013 and is now providing services for virtual machines, bare-metal and containers within a single cloud. Containers run on either virtual machines or bare-metal depending on the use cases, all provisioned via OpenStack Magnum. A selection of different container technologies are available including Kubernetes, Docker Swarm and DC/OS.</p>',
          '<p>CERN is currently running 250 container clusters provisioned through Magnum on top of OpenStack.</p>',
          '<p class="text-center"><img class="medium-img" src="https://www.openstack.org/themes/openstack/images/containers2/cern-diagram.jpg" alt="Cern"></p>',
...
<p>CERN uses federations of Kubernetes clusters to support multi-cloud operations. Multiple clusters can be seamlessly integrated across clouds of varying technologies, including AWS, GCE and OpenStack clouds such as CERN and the T-Systems Open Telekom Cloud <a href="https://www.youtube.com/watch?v=2PRGUOxL36M" target="_blank">as demonstrated at Kubecon 2018</a>.</p>
        ...
        $idx = 1;
        foreach($case2_contents as $content){
            $c = new CaseOfStudyParagraph();
            $c->Content = self::cleanContent($content);
            $c->Order = $idx;
            $c->CaseOfStudyID = $case2->ID;
            $c->write();
            $idx++;
        }

        // SK Telecom
        $case3 = new CaseOfStudy();
        $case3->Title = "SK Telecom";
        $case3->Order = 3;
        $case3->SectionID = $section4->ID;
        $case3->write();

        $case3_contents = [
            '<p>SK Telecom (SKT), South Korea’s largest telecommunications operator, has been exploring optimized approaches for deploying OpenStack on Kubernetes with the aim of putting core business functions on containerized OpenStack by the end of 2018. SKT leverages Kolla and Openstack-Helm. with deployments automated by Kubespray. SKT devotes nearly 100% of it’s development efforts to OpenStack-Helm, and works closely with AT&T to make OpenStack-Helm successful.</p>',
            ...
            '<p class="text-center"><img class="medium-img" src="https://www.openstack.org/themes/openstack/images/containers2/sk-diagram.jpg" alt="SK Telecom"></p>',
...
<p>Atelconetworkinfrastructurebuiltonvirtualnetworkfunctions(plannedtoopensometimein2019)<p>
        ...
        $idx = 1;
        foreach($case3_contents as $content){
            $c = new CaseOfStudyParagraph();
            $c->Content = self::cleanContent($content);
            $c->Order = $idx;
            $c->CaseOfStudyID = $case3->ID;
            $c->write();
            $idx++;
        }

        // Superfluidity
        $case4 = new CaseOfStudy();
        $case4->Title = "Superfluidity";
        $case4->Order = 4;
        $case4->SectionID = $section4->ID;
        $case4->write();
        ...
        $case4_contents = [
            ...
            '<p class="text-center"><img class="medium-img" src="https://www.openstack.org/themes/openstack/images/containers2/super-table.jpg" alt="Superfluidity"></p>',
        ...
        $section_6_items = [
            'Airship' => '<p>Airship is a collection of interoperable and loosely coupled open source tools that provide automated cloud provisioning and management in a declarative way, based around Kubernetes as an application platform.</p>',
            'Ansible' => '<p>Ansible is a commonly used orchestration tool used to deploy and manage OpenStack installations.</p>',
            'Cinder' => '<p>OpenStack Cinder offers block storage as a service, providing a single API backed by over seventy different possible storage drivers.</p>',
            'Cloud Provider OpenStack' => '<p>Cloud Provider OpenStack is the implementation of the Kubernetes Cloud Provider interface. It allows an OpenStack-hosted Kubernetes cluster to directly access storage and load balancer resources in the OpenStack cloud.</p>',
            'Calico' => '<p>Calico is a network overlay with drivers for both Kubernetes and OpenStack that features L3-only routing.</p>',
            'Cyborg' => '<p>Cyborg is an OpenStack project that provides a general management framework for hardware accelerators including FPGA, GPU, ASIC, and others. Work is in progress to surface a general hardware interface to pods.</p>',
            'Docker' => '<p>Docker is an open source container virtualization framework, used to host containerized applications.</p>',
            'Helm' => '<p>Helm is the official package manager for Kubernetes. Application deployments are described by Helm-Charts, which can be automatically deployed and managed on a Kubernetes cluster.</p>',
            'Ironic' => '<p>Ironic is the OpenStack bare-metal service. Running either as a standalone service or as a driver to OpenStack Nova, it can manage the complete life-cycle of bare-metal systems, including enrollment, provisioning, maintenance, and decommissioning.</p>',
            'Loci' => '<p>LOCI is an OpenStack project to build lightweight, OCI compliant containers for OpenStack projects.</p>',
            'LXC' => '<p>LXC is a low-level container virtualization interface that takes advantage of Linux kernel namespace isolation and other technologies to create isolated linux runtimes.</p>',
            'Kata Containers' => '<p>Kata Containers is a standard implementation of lightweight Virtual Machines (VMs) that feel and perform like containers, but provide the workload isolation and security advantages of VMs.</p>',
            'Keystone' => '<p>Keystone is the OpenStack Identity service that provides means for authenticating and managing user accounts and role information primarily for the OpenStack cloud environment, but also as a plugin to other environments, including Kubernetes.</p>',
            'Kolla (Containers)' => '<p>Kolla (Containers) is an OpenStack project to build containers for each OpenStack service. It includes a sophisticated build and templating systems, and is capable of building containers from both source and packages on a variety of host operating systems.</p>',
            'Kolla Ansible' => '<p>Kolla Ansible is an OpenStack project that uses Ansible to deploy and maintain a full OpenStack installation using Kolla containers.</p>',
            'Kubernetes' => '<p>Kubernetes is a container orchestration system that delivers robust and highly-available applications on top of cloud-infrastructure.</p>',
            'Kuryr' => '<p>Kuryr is an OpenStack project that provides a Neutron network overlay to container runtimes, including Docker and Kubernetes. It aims to be the “integration bridge” for container and VM networks.</p>',
            'Magnum' => '<p>Magnum is an OpenStack project that offers managed container platforms as a service, including Kubernetes, Docker Swarm, Mesos, and DC/OS platforms. It is capable of creating tenant isolated application platforms through a simple user-facing API.</p>',
            'Neutron' => '<p>Neutron is the OpenStack software-defined networking service, offering a single API to deliver dynamic network infrastructure backed by dozens of network drivers.</p>',
            'OpenStack Ansible' => '<p>OpenStack Ansible is a project for building OpenStack services into LXC containers, and for deploying and managing OpenStack installations within those containerized services.</p>',
            'OpenStack Helm' => '<p>OpenStack Helm is an OpenStack project that deploys and manages the lifecycle of OpenStack and supporting infrastructure on top of Kubernetes (eg Ceph and MariaDB) , delivering production ready deployments, for a range of use cases from small edge deployments to large central offices. Leveraging the Helm package management system. OpenStack Helm has support for both baremetal (Ironic) and virtual (Nova/KVM) workload management, and is image agnostic supporting both LOCI and Kolla containers.</p>',
            'Qinling' => '<p>Qinling is an OpenStack project to deliver Functions as a Service. Qinling supports different container orchestration platforms, such as Kubernetes and Docker Swarm, as well as different function package storage backends such as local file-store, OpenStack Swift, and S3.</p>',
            'Triple-O' => '<p>TripleO is a project aimed at installing, upgrading and operating OpenStack clouds using OpenStack’s cloud services as the foundation - building on Nova, Ironic, Neutron, Heat and Ansible to automate cloud management.</p>',
            'Zun' => '<p>Zun is the OpenStack Containers service. It aims to provide an API service for running application containers without the need to manage servers or clusters. </p>',
        ];

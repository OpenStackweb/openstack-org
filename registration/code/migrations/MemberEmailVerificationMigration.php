...
final class MemberEmailVerificationMigration extends AbstractDBMigrationTask
{
    ...
    function doUp()
    {
        ...
        if(intval(PermamailTemplate::get()->filter('Identifier', MEMBER_REGISTRATION_VERIFICATION_EMAIL_TEMPLATE_ID)->count()) === 0 )
        {
            ...
            $email_tpl->Subject    = 'Thank you for becoming an Open Infrastructure Foundation Member';
            $email_tpl->From       = 'noreply@openstack.org';
            $email_tpl->Content    ='<p>Dear {$Member.FullName}, thank you for joining the Open Infrastructure Foundation.</p><p>IMPORTANT! Please click <a href="{$VerificationLink}">here</a> to complete your registration.</p>';
            $email_tpl->write();
        }
...
<ul><li>Meet the <a href="http://www.openstack.org/foundation/staff">Open Infrastructure Foundation Staff</a>, the <a href="http://www.openstack.org/foundation/board-of-directors/">Board of Directors</a>, and the <a href="http://www.openstack.org/foundation/technical-committee/">Technical Committee</a>.</li>

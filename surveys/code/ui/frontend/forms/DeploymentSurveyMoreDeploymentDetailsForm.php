...
class DeploymentSurveyMoreDeploymentDetailsForm extends Form
{
    ...
    function __construct($controller, $name)
    {
        ...
        if(strpos($projects_used, 'Compute (Nova)') > 0){
            $fields->add(
                new CustomCheckboxSetField(
                    'Hypervisors',
        ...
        if(strpos($projects_used, 'Networking (Neutron)') > 0){
            $fields->add(
                new CustomCheckboxSetField(
                    'NetworkDrivers',
        ...
        if(strpos($projects_used, 'Identity (Keystone)') > 0){
        ...
        if(strpos($projects_used, 'Block Storage (Cinder)') > 0){
            $fields->add(new CustomCheckboxSetField(
                'BlockStorageDrivers',
        ...
        if(strpos($projects_used, 'Object Storage (Swift)') > 0){
            $fields->add($ddl_block_size = new DropdownField(
                'ObjectStorageSize',

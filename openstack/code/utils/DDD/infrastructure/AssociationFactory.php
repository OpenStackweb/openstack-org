...
final class AssociationFactory
{
    ...
    public function getOne2ManyAssociation(DataObject $entity, $association_name, QueryObject $query = null)
    {
        ...
        if (!$child_class) {
            throw new Exception(sprintf("entity %s has not an one-to-many association called %s", get_class($entity),
                $association_name));
        }
    ...
    public function getMany2ManyAssociation(DataObject $entity, $association_name, QueryObject $query = null)
    {
        ...
        if (!$componentClass) {
            throw new Exception(sprintf("entity %s has not a many-to-many association called %s", get_class($entity),
                $association_name));
        }
     ...
     * @throws Exception
     */
    public function getMany2OneAssociation(
        DataObject $entity,
        $association_name,
        $inversed_by = null,
        QueryObject $target_query = null
    ) {
        $class_name = $entity->has_one($association_name);
        if (!$class_name) {
            throw new Exception(sprintf("entity %s has not an many-to-one association called %s", get_class($entity),
                $association_name));
        }

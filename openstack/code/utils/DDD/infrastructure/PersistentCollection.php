...
final class PersistentCollection extends RelationList
{
    ...
    function remove($item)
    {
        ...
        if (!$deleted) {
            foreach ($this->items as $old_item)
            {
                if (get_class($item) === get_class($old_item) && $old_item->ID === $item->ID)
                {
                    unset($this->items[spl_object_hash($old_item)]);
                    $deleted = $this->is_dirty = true;
                    break;
                }
            }

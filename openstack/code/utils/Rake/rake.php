...
class Rake
{
	 ...
	 */
	function __construct($stopwords_path = BASE_PATH.'/openstack/code/utils/Rake/stoplist_smart.txt')
	{
     ...
     * @param string $text Input text
     */
    public function extract_words($text)
    {
        $phrases_plain = self::split_sentences($text);
        $phrases = $this->get_phrases($phrases_plain);
        $words = $this->get_frequency($phrases);
        arsort($words);
     ...
     * @param array $phrases Array containing individual phrases
     */
    private function get_frequency($phrases)
    {
        $frequencies = array();
